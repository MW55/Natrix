...
rule all:
    input:
        ...
        "results/finalData/taxonomy_splits/"+str(config["blast"]["annotate_rank"])+"_annotated.csv" if config["blast"]["blast"] and config["blast"]["database"] == "NCBI" and config["blast"]["annotate_rank"] != None else []
