...
if snakemake.params.paired_end:
    if snakemake.params.prim_rm:
        ...
        subprocess.call(["cutadapt","-j 0", "-g", r1_primer + r1_barcode, "-G", r2_primer + r2_barcode, "-o",
                         snakemake.output[0], "-p", snakemake.output[1], snakemake.input[0], snakemake.input[1]])
    else:
        subprocess.call(["cutadapt","-j 0", "-g", r1_primer, "-G", r2_primer, "-o",
                         snakemake.output[0], "-p", snakemake.output[1], snakemake.input[0], snakemake.input[1]])
else:
    if not snakemake.params.bar_removed:
        subprocess.call(["cutadapt","-j 0", "-g", r1_primer + r1_barcode, "-o", snakemake.output[0], snakemake.input[0]])
    else:
        subprocess.call(["cutadapt","-j 0", "-g", r1_primer, "-o", snakemake.output[0], snakemake.input[0]])