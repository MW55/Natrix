...
# Parsing the whole fasta to a dict, at this point in the pipeline the files
# are small enugh to fit into memory and using a dict comp with dinopy
# is quicker then the to_dict func from SeqIO
...
# returns a list of tuples containing the old header of the representative
# seq of each cluster and the new header as required by uchime
...
# writes the representatives in a fasta file with the new headers, uses the old header as key
# to get the matching sequences from the dict
