...
import pandas as pd
import re
taxonomy = pd.read_csv(snakemake.input[1], sep='\t', names=['seqid', 'taxonomy']) #read mothur taxonomy file
#print(taxonomy.head())
swarm_count = pd.read_csv(snakemake.input[0]) #read swarm output file
print (type(taxonomy))
...
final_file.to_csv(snakemake.output[0], index=False) #output file
